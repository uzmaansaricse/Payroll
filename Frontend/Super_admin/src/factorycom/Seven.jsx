...
const defaultPermissions = {
  'Maintenance Lead': {
    'Monitor Equipment': true,
    'Approve Maintenance Requests': true,
    'Manage Maintenance Team': true,
    'View Maintenance Reports': true,
  },
  'QC Lead': {
    'Conduct Quality Checks': true,
    'Approve Quality Reports': true,
    'Manage QC Team': true,
    'View QC Reports': true,
  },
  'Storekeeper': {
    'Manage Inventory': true,
    'Approve Stock Requests': true,
    'Track Stock Levels': true,
    'View Inventory Reports': true,
  },
  'Safety Officer': {
    'Monitor Safety Protocols': true,
    'Conduct Safety Audits': true,
    'Manage Safety Training': true,
    'View Safety Reports': true,
  },
};

export default function RolePermissionsManager3() {
  ...
  const [employees, setEmployees] = useState({
    1: { 'Maintenance Lead': [], 'QC Lead': [], 'Storekeeper': [], 'Safety Officer': [] },
    2: { 'Maintenance Lead': [], 'QC Lead': [], 'Storekeeper': [], 'Safety Officer': [] },
  });
  ...
  const handleAddRole = () => {
    ...
    setRolesData(prev => ({
      ...
      [selectedFactory]: {
        ...
        [newRole]: {
          'Monitor Equipment': false,
          'Approve Maintenance Requests': false,
          'Manage Maintenance Team': false,
          'View Maintenance Reports': false,
        },
      },
    }));
    setNewRole('');
  ...
  const handleAddEmployee = () => {
    const { name, email, phone, role } = employeeForm;
    ...
    setEmployees(prev => ({
      ...
      [selectedFactory]: {
        ...prev[selectedFactory],
        [role]: [
          ...(prev[selectedFactory][role] || []),
          { name, email, phone },
        ],
      },
    }));

    setEmployeeForm({ role: '', name: '', email: '', phone: '' });
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-8">
      ...
      {/* Add Employee */}
      <div className="bg-white shadow rounded p-6">
        <h3 className="text-lg font-semibold mb-4">Add Employee (Maintenance Lead, QC Lead, Storekeeper, Safety Officer)</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          >
            ...
            {Object.keys(defaultPermissions).map((role, i) => (
        ...
        {employees[selectedFactory] && Object.entries(employees[selectedFactory]).map(([role, employeeList], idx) => (
          employeeList.length > 0 && (
            <div key={idx} className="mt-6">
              <h4 className="font-medium text-gray-700 mb-2">{role}</h4>
              <ul className="list-disc pl-5 space-y-1 text-sm text-gray-800">
                {employeeList.map((employee, i) => (
                  <li key={i}>{employee.name} — {employee.email} — {employee.phone}</li>
                ))}
              </ul>
            </div>
          )
                ...
                <table className="min-w-full border text-sm">
                  <thead>
                    <tr className="bg-gray-100">
                      ...
                      {Object.keys(defaultPermissions['Maintenance Lead']).map((perm, i) => (
                    ...
                    {submittedData.roles[factory.id] &&
                      Object.entries(submittedData.roles[factory.id]).map(([role, perms], i) => (
                        <tr key={i}>
                          ...
                          {Object.keys(defaultPermissions['Maintenance Lead']).map((perm, j) => (
