...
const dummyFactories = [
  { id: "F001", name: "Factory 1" },
  { id: "F002", name: "Factory 2" },
  { id: "F003", name: "Factory 3" },
];
...
const dummyHolidays = {
  F001: [
    ...
    { id: 2, date: "2025-10-02", name: "Gandhi Jayanti" },
  ],
  F002: [{ id: 1, date: "2025-12-25", name: "Christmas" }],
  F003: [],
};

// Merge real holidays with factory holidays
for (const factory of dummyFactories) {
  if (!dummyHolidays[factory.id] || dummyHolidays[factory.id].length === 0) {
    dummyHolidays[factory.id] = real2025Holidays.map((h, index) => ({
      id: Date.now() + index,
      date: h.date,
      name: h.name,
    }));
...
export default function FactoryHolidaySetup() {
  const [selectedFactory, setSelectedFactory] = useState("F001");
  const [holidayData, setHolidayData] = useState(dummyHolidays);

  const handleChange = (index, field, value) => {
    const updated = [...holidayData[selectedFactory]];
    updated[index][field] = value;
    setHolidayData({ ...holidayData, [selectedFactory]: updated });
  };

  const handleAdd = () => {
    const updated = [
      ...holidayData[selectedFactory],
  ...
  const handleDelete = (id) => {
    const updated = holidayData[selectedFactory].filter((h) => h.id !== id);
    setHolidayData({ ...holidayData, [selectedFactory]: updated });
  };
  ...
  const isHoliday = (date) => {
    ...
    return holidayData[selectedFactory].some((h) => h.date === formatted);
  };

  const getHolidayName = (date) => {
    const formatted = date.format("YYYY-MM-DD");
    const holiday = holidayData[selectedFactory].find(
      (h) => h.date === formatted
    );
    return holiday?.name || "";
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white rounded-2xl shadow space-y-8">
      <h2 className="text-2xl font-semibold">Factory Holidays Setup</h2>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full table-auto border mb-4">
          ...
          <tbody>
            {holidayData[selectedFactory]?.map((holiday, index) => (
            ...
            {holidayData[selectedFactory].length === 0 && (
              <tr>
