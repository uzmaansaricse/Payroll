...
const FactoryAttendanceAssign2 = () => {
  ...
  const [numberToSelect, setNumberToSelect] = useState('');
  ...
  return (
    <div className="max-w-5xl mx-auto p-8">
      ...
      {selectedDepartment && (
        <>
          <div className="my-4">
            <label className="block mb-2">Select Number of Employees</label>
            <input
              type="number"
              value={numberToSelect}
              onChange={(e) => {
                const value = parseInt(e.target.value, 10);
                setNumberToSelect(e.target.value);
                if (!isNaN(value) && value > 0) {
                  const selectedIds = filteredEmployees.slice(0, value).map(emp => emp.id);
              ...
              className="w-full px-4 py-2 border rounded-md"
              placeholder="Enter number of employees to auto-select"
            />
          </div>
...
export default FactoryAttendanceAssign2;
