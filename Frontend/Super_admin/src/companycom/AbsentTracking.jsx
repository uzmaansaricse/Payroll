...
const LeaveAssignment = () => {
  ...
  const [leaveTypes] = useState(['PL', 'SL', 'CL']);
  ...
  const handleLeaveTypeSelection = (e) => {
    const { value, checked } = e.target;
    if (checked) {
      setSelectedLeaveTypes((prev) => [...prev, value]);
    } else {
      setSelectedLeaveTypes((prev) => prev.filter((leaveType) => leaveType !== value));
    }
  };

  const handleSubmit = () => {
    if (!selectedAuthority || selectedEmployees.length === 0 || selectedLeaveTypes.length === 0) {
    ...
    setSubmittedData((prevData) => [...prevData, submittedAssignment]);
  ...
  const handleApproval = (index, authority) => {
    setSubmittedData((prevData) => {
      ...
      const assignment = updatedData[index];

      // Update the approval status based on the authority
      if (assignment.approvalStatus === 'Pending') {
        assignment.approvalStatus = `Approved by ${authority}`;
      } else {
        alert('This leave request has already been approved.');
      }
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      <div className="p-5 mb-6 border rounded-lg shadow-md">
        ...
        <div className="my-4">
          <label htmlFor="leaveTypes" className="block mb-2 text-gray-700">
            Select Leave Types
          </label>
          <div className="flex flex-wrap">
            {leaveTypes.map((leave) => (
              <div key={leave} className="mr-4">
                <input
                  type="checkbox"
                  id={leave}
                  value={leave}
                  checked={selectedLeaveTypes.includes(leave)}
      ...
      {/* Display submitted leave assignments in a table */}
      <div className="mt-8">
